...
本目录包含 OpenClaw 在 Linux / macOS 下的安装相关脚本：

- `openclaw/install-release.sh` — **OpenClaw 官方安装脚本**（macOS + Linux + WSL）。
- `openclaw/install-github.sh` — 直接从 GitHub Release 下载安装。
- `install-user.sh` — 本仓库提供的用户态一键安装入口。
